...
{showContent && <div className='p-12 right-0 z-50 absolute w-8/12 space-y-4 flex flex-col items-center justify-center h-screen'>  
      <div className="bg-black bg-opacity-50 p-4 rounded-lg space-y-12 w-full">
      <div className="space-y-4">
        {faqData.map((item) => (
          <div key={item.id}>
            <div className='space-y-10'>
              <Link href={item.link} target="_blank" rel="noopener noreferrer" className='hover:bg-wlOrange'>
                <h4 className='text-white hover:text-wlOrange'>{item.id}. {item.question}</h4>
              </Link>
            </div>
            <hr />
          </div>
